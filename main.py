...
def _get_site_identity(d: Dict[str, Any]) -> tuple[Optional[str], Optional[str]]:
    ...
    log.debug(f"[_get_site_identity] Initial: site_id={site_id}, site_token={site_token}")
    if isinstance(d, dict) and "events" in d and isinstance(d["events"], list) and d["events"]:
        ...
        log.debug(f"[_get_site_identity] From first event: site_id={site_id}, site_token={site_token}")
    
    final_site_id = str(site_id) if site_id else None
...
def _get_identity_info(d: Dict[str, Any]) -> tuple[bool, Optional[str], Optional[str], Optional[str], int]:
    ...
    log.debug(f"[_get_identity_info] Payload fields: site_id={site_id}, site_token={site_token}, shop={shop}")
    ...
    if isinstance(evs, list):
        ...
        if evs and isinstance(evs[0], dict):
             ...
             log.debug(f"[_get_identity_info] After first event override: site_id={site_id}, site_token={site_token}, shop={shop}")
...
def _overwrite_site_id(payload: dict, site_id: str) -> None:
    ...
    log.info(f"[_overwrite_site_id] Payload site_id overwritten from {original_sid} to {site_id}")
    ...
    if isinstance(evs, list):
        ...
        for i, ev in enumerate(evs):
            if isinstance(ev, dict):
                ...
                ev["site_id"] = site_id
                if original_ev_sid != site_id:
                     log.debug(f"[_overwrite_site_id] Event {i} site_id changed from {original_ev_sid} to {site_id}")
            else:
...
def _validate_site_origin_strict(site_id: str, request: Request, payload: Dict[str, Any]) -> bool:
    ...
    log.debug(f"[validate_origin] Starting strict check for resolved site_id={site_id}")
    ...
    log.debug(f"[validate_origin] Allowed domains for {site_id}: {sorted(allowed)}")
    
    if not allowed:
        log.warning(f"[validate_origin] REJECTED site_id={site_id}: No allowed domains configured in SITE_URL_MAP.")
    ...
    for candidate in candidate_hosts:
        if candidate in allowed:
            log.info(f"[validate_origin] ✅ ACCEPTED site_id={site_id} via domain match: {candidate}")
            return True

    log.warning(f"[validate_origin] ❌ REJECTED site_id={site_id} | No candidate host matched allowed: {sorted(allowed)}")
    return False
...
async def collect_single(request: Request):
    ...
    if not is_legacy:
        ...
        log.info(f"[collect] legacy host pass-through accepted for site_id={site_id}")


    if API_SECRET and not _validate_secret(data):
        log.warning(f"[collect] UNAUTHORIZED single (bad secret) site_id={site_id}")
